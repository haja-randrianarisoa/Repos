...
var jadeTemplateIndexHtml =
   ...
   // "       button(type='button',target='_blank',onClick='location.href=(../../../Logs/logFile.log)') View Log\n"+
    ...
    "                td.os-col(style='width:10%;overflow-y: hidden;text-align:center;word-wrap:break-word')\n"+
    "                  span= suite.osName \n"+
    ...
    "                td.img-col(style='width:10%;text-align:center')\n"+
    "                  a(class='button',href=suite.screenshotUrl+suite.browserName +'-' + suite.name +' '+ testcase.name +'-expect failure-'+0+'.png',target='_blank') View\n"+
    "\n"+
...
var  p =path.resolve(path.join(__dirname,"../../Reports/Xmlfiles"));
var indexHtmlPath = path.resolve( path.join(__dirname,"../../Reports/Recent/Htmlreport/FinalReport.html"));
...
fs.readdir(p,function(err,files ){
    if(err)
        ...
        backup();
        files.forEach(function (file) {
            var xml = fs.readFileSync(path.join(p,'/')+ file, {encoding: 'utf-8'});// reading xml files
            ...
            parser.parseString(xml, function (err, json) {
                ...
                for (var i = 0; i < testsuiteArray.length; i++) {
                    ...
                    var suite = {
                        browserName: divide[0].split('.')[0],
                        browserVersion:divide[0].split('.')[1],
                    ...
                    let len=testcaseArray===undefined?0:(testcaseArray.length);
                    for (var x = 0; x <= len && testcaseArray!==undefined; x++) {
                        if (!testcaseArray[x])
                            continue;
